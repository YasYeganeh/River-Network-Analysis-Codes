...
y=np.array([len(a1),len(a2),len(a3),len(a4)])
log_y = np.log(y)
...
allbranches_List=branch1_List+branch2_List+branch3_List+branch4_List
...
plt.figure()
x=np.array(list(range(min_order,max_order+1,1)))
y=np.array([L_avg1,L_avg2,L_avg3,L_avg4])
log_y = np.log(y)
coefficients = np.polyfit(x, log_y, 1)
# print(coefficients)
...
# print(coefficients[0],coefficients[1])
c = np.exp(coefficients[1]) * np.exp((coefficients[0])*x)
plt.text(1, 100000, f'Ln(y)={coefficients[0]}*x + {coefficients[1]}',c='b')
plt.title("Rl")
plt.plot(x, c,c='b')
plt.scatter(x, y)
plt.xlabel("Order", fontsize=15)
...
plt.figure()
x=np.array(list(range(min_order,max_order+1,1)))
y=np.array([Area1_avg,Area2_avg,Area3_avg,Area4_avg])
log_y = np.log(y)
coefficients = np.polyfit(x, log_y, 1)
# print(coefficients)
...
# print(coefficients[0],coefficients[1])
c = np.exp(coefficients[1]) * np.exp((coefficients[0])*x)
plt.text(1, 990000000, f'Ln(y)={coefficients[0]}*x + {coefficients[1]}',c='coral')
plt.title("Ra")
